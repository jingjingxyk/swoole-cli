...
UNIX_DOWNLOAD_SWOOLE_CLIE_RUNTIME() {
  ...
  local ARCH="$2"
  local APP_VERSION="$3"
  local APP_DOWNLOAD_URL="https://github.com/jingjingxyk/build-static-coturn/releases/download/${VERSION}/${APP_NAME}-${APP_VERSION}-${OS}-${ARCH}.tar.xz"
...
WINDOWS_DOWNLOAD_SWOOLE_CLIE_RUNTIME() {
  local APP_VERSION="$1"
  local ARCH="x64"
  local APP_DOWNLOAD_URL="https://github.com/jingjingxyk/build-static-coturn/releases/download/${VERSION}/${APP_NAME}-${APP_VERSION}-cygwin-${ARCH}.zip"

  local APP_RUNTIME="${APP_NAME}-${APP_VERSION}-cygwin-${ARCH}"
  test -f ${APP_RUNTIME}.zip || curl -fSLo ${APP_RUNTIME}.zip ${APP_DOWNLOAD_URL}

  APP_DOWNLOAD_URL="https://github.com/jingjingxyk/build-static-coturn/releases/download/${VERSION}/${APP_NAME}-${APP_VERSION}-msys2-${ARCH}.zip"
