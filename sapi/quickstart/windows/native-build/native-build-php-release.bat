...
rem echo 	^@"$(LINK)" ^/nologo $(PHP_GLOBAL_OBJS) $(PHP_GLOBAL_OBJS_RESP) $(CLI_GLOBAL_OBJS) $(CLI_GLOBAL_OBJS_RESP)  $(STATIC_EXT_OBJS) $(STATIC_EXT_OBJS_RESP)  $(ASM_OBJS) $(LIBS) $(LIBS_CLI) $(BUILD_DIR)^\php.exe.res /out:$(BUILD_DIR)^\php.exe $(LDFLAGS) $(LDFLAGS_CLI) $(STATIC_EXT_LIBS)   >> %x_makefile%
echo 	^@"$(LINK)" ^/nologo $(PHP_GLOBAL_OBJS) $(CLI_GLOBAL_OBJS) $(CLI_GLOBAL_OBJS_RESP) $(STATIC_EXT_OBJS) $(ASM_OBJS) $(LIBS) $(LIBS_CLI) $(BUILD_DIR)^\php.exe.res /out:$(BUILD_DIR)^\php.exe $(LDFLAGS) $(LDFLAGS_CLI) $(STATIC_EXT_LIBS)   >> %x_makefile%
