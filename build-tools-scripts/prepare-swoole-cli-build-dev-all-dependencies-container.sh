...
docker build -t ${IMAGE} -f ./Dockerfile-alpine-swoole-cli-build-dev-all-dependencies  . --progress=plain --build-arg PROXY_URL=$PROXY_URL

echo ${IMAGE} > swoole-cli-build-dev-all-dependencies-container.txt
docker push ${IMAGE}
